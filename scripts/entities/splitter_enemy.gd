...
func _apply_profile_from_balance() -> void:
    var profile: Dictionary = BalanceService.get_enemy_profile("splitter")
...
func get_display_name() -> String:
    return "Splitter Glitch"

func _draw() -> void:
    if not _visual_has_sprite:
        draw_circle(Vector2.ZERO, body_radius + 2.0, Color(0.14, 0.03, 0.18, 0.92))
        draw_circle(Vector2.ZERO, body_radius, Color(1.0, 0.28, 0.78, 1.0))
        draw_arc(Vector2.ZERO, body_radius + 4.0, 0.2, PI * 1.7, 24, Color(1.0, 0.72, 0.95, 0.72), 1.4)
    ...
    draw_rect(Rect2(bar_pos, Vector2(bar_width, bar_height)), Color(0.16, 0.06, 0.16, 0.9), true)
    draw_rect(Rect2(bar_pos, Vector2(bar_width * hp_ratio, bar_height)), Color(1.0, 0.38, 0.86, 1.0), true)
